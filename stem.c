...
char * startStemTerm( char * termArray, int termSize )
{
   // OLD
   // z = struct
   // s = char array (term)
   // i = size of the term. (size_t of s)
   // ==================================== //
   struct stemmer * z = create_stemmer();

   // printf( "Regular Term: %s \r\n", termArray );
   termArray[ stem( z, termArray, termSize - 1 ) + 1 ] = 0;
   // printf( "Stemmed Term: %s \r\n", termArray );

   // ======= CLEAR ======== //
   // memset(termArray, 0, sizeof termArray);
   return termArray;
}
